...
if (isset($_POST['register'])) {
    ...
    $validar = mysqli_query($conn,"SELECT * FROM administrativos WHERE Correo='$Email';");
    if (empty($_POST['Nombre']) && empty($_POST['Apellido1']) && empty($_POST['Apellido2']) && empty($_POST['Correo']) && empty($_POST['Puesto'])){
        echo "<p class='error'>No puedes dejar campos vacios</p>";
    }else{
        if(mysqli_num_rows($validar)>0){
            echo "<p class=error>El correo ya esta registrado</p>";
            exit();
        }else{
            $insertar = mysqli_query($conn,"INSERT INTO administrativos (`Nombre`,`Apellido1`,`Apellido2`,`Correo`,`Puesto`) VALUE ('$Nom','$A1','$A2','$Email','$puesto');");
