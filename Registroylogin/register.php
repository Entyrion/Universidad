...
if (isset($_POST['register'])) {
    ...
    // $validar = mysqli_query($connection,"SELECT * FROM alumnos WHERE Correo='$Email';");
    // if(mysqli_num_rows($validar)>0){
    //     echo "<p class=error>El correo ya esta registrado</p>";
    //     exit();
    // }else{
    //     $insertar = mysqli_query($connection,"INSERT INTO alumnos (`Nombre`,`Apellido1`,`Apellido2`,`Carrera`,`Correo`) VALUE ('$Nom','$A1','$A2','$Carr','$Email','$password');");
    ...
    $validar = mysqli_query($connection,"SELECT * FROM usuarios WHERE USUARIO='$Usr' OR EMAIL='$Email'");
    if(mysqli_num_rows($validar)>0){
        echo '<p class="error">El usuario o el correo ya esta registrado</p>';
        exit();
    }else{
        $insertar = mysqli_query($connection,"INSERT INTO usuarios VALUE ('$Email','$Usr','$Pswd');");
        echo '<p class="success">Registro exitoso</p>';
        exit();
    }
}
    ...
    <div class="container-fluid">
        ...
        <form method="post" action="" name="signup-form">
            <div class="form-element">
                <label>Usuario</label>
                <input type="text" name="Usr" pattern="[a-zA-Z0-9]+" required />
            </div>
            <div class="form-element">
                <label>Correo</label>
        ...
        <div class="t-30">
        <p>O<a class="linkreg" href="login.php"> Ingresa</a></p>
        </div>
